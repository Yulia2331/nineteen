...
export default function MarketingItem(props) {
  ...
  return (
    ...
    `} >
        ...
        `} >
                    <h3 className="text-xl sm:text-2xl lg:text-3xl text-white uppercase font-semibold">{props.title}</h3>
                ...
                <div className={`card-back h-full w-full sm:aspect-square absolute left-0  top-0 z-20  transition-all duration-[.8s] rotate-y-180 opacity-0 `}>
                    <div className={`h-full bg-white xs:px-5 pt-12 rounded-[20px] shadow-[0_9px_33px_rgba(36,36,36,0.2)] ${props.class}`}>
                        <h3 className="text-xl sm:text-2xl lg:text-3xl mb-3 uppercase font-semibold">{props.title}</h3>
                        <p className="text-[16px] sm:text-xl lg:text-2xl">{props.text}</p>
                    </div>
                </div>
        </div>
        );
